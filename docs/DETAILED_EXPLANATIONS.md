...
4. NOP insertion, dead-code insertion, constant-pool randomization and opcode
   remapping all run at any non-zero level, including the CLI default.
   `--mutation 0` is the only setting that leaves the program byte-identical.
5. `ReorderInstructions` has been removed. It needed a reverse mapping the VM
   cannot carry, and a config field with no implementation behind it reads as
   a stage that is merely switched off.
