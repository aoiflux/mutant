...
var builtinDocs = map[string]builtinDoc{
	...
	BuiltinNameViewList: {
		...
		returns:   pairRet("the postures in force", ParamHash).withFields("case_id", "count", "open", "tagged", "views")},
	BuiltinNameViewPreview: {
		signature: "view_preview(record, view)",
		summary:   "Says what a view would disclose from a record, before anything is disclosed. It is arithmetic over the record's public header and the case's class table: it opens no segment, needs no plaintext, and `reads_no_plaintext` is a field saying so. `granted_runs` and `withheld_runs` are the byte ranges on each side, merged where adjacent segments of one class sit next to each other, and both are reported because \"what am I holding back\" is the question a disclosure review actually asks. Where the record was sealed by `record_seal_quantised`, `rounding_effect` states the direction in the terms of this view: rounding moves bytes into one named class, so the same rounding releases more when the view grants that class and withholds more when it does not, and a count printed without a direction is one a reviewer completes themselves. A class the record carries whose tag this case cannot name is counted in `unnamed_classes` rather than passed off as unclassified. Returns (preview, err).",
		params: []builtinParamDoc{
			param("record", "Handle from record_open.", ParamInt),
...
var capabilityCategories = []capabilityCategory{
	...
	// view_ is the disclosure half of the same story and gets its own heading
	// rather than joining "classified records": one family says how evidence is
