...
var builtinDocs = map[string]builtinDoc{
	...
	BuiltinNameMftParse:       {signature: "mft_parse(path)", summary: "Parses an NTFS Master File Table into a per-record timeline. Auto-detects a standalone $MFT file (FILE-signature record stream, e.g. KAPE/FTK/icat) vs a full NTFS volume image. Each entry has $STANDARD_INFORMATION and $FILE_NAME MAC times (unix + iso), reconstructed path, size, sequence, and hard-link count. The record size is read from the first record header rather than assumed, and skipped counts records that would not parse. Returns {source_type, record_size, count, skipped, entries:[{record, parent_record, in_use, is_directory, name, path, size, allocated_size, sequence, hard_links, file_attributes, si_*, fn_*}]}. Returns (result, err).", params: []builtinParamDoc{{name: "path", doc: "Path to a standalone $MFT file or an NTFS volume image."}}},
